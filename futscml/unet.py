...
from .nn import (
    ...
    zero_module,
    normalization,
    timestep_embedding, normalization2d,
)
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        self.in_layers = nn.Sequential(
            normalization2d(channels, self.conf),
            nn.SiLU(),
        ...
        self.out_layers = nn.Sequential(
            normalization2d(self.out_channels, self.conf),
            nn.SiLU(),
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm = normalization_(channels, self.conf)
        self.to_q = conv_nd(1, channels, channels, 1)
        self.to_k = conv_nd(1, channels, channels, 1)
        self.to_v = conv_nd(1, channels, channels, 1)
    ...
    def forward(self, x, att=None):
        return checkpoint(self._forward, (x, att), self.parameters(), True)

    def _forward(self, x, att=None):
        b, c, *spatial = x.shape
        ...
        if self.conf is not None and self.conf['attention_version'] == 'v3':
            k, q, v = self.get_qkv_v3(a, att, b, c)
        elif self.conf is not None and self.conf['attention_version'] == 'v2':
            k, q, v = self.get_qkv_v2(a, att, b, c)
        elif self.conf is not None and self.conf['attention_version'] == 'v1':
            k, q, v = self.get_qkv_v1(a, att, b, c)
        else:
            raise RuntimeError('Unknown attention version')

        qkv = th.cat((q, k, v), dim=1)

        heads = self.attention(qkv)

        heads = self.proj_out(heads)
        return (x + heads).reshape(b, c, *spatial)

    def get_qkv_v1(self, a, att, b, c):
        q = self.to_q(a)
        if att is not None:
            att = att.reshape(b, c, -1)
            k = self.to_k(att)
            v = self.to_v(att)
        else:
            k = self.to_k(a)
...
class UNetModel(nn.Module):
    ...
    def __init__(self, image_size, in_channels, model_channels, out_channels, num_res_blocks,
                 ...
                 resblock_updown=False, use_new_attention_order=False, conf=None,
                 temporal_super_resolution_steps=1):
        super().__init__()
        ...
        self.diffusion_steps_including_temporal_super_resolution_steps = \
            self.conf.diffusion_steps if self.conf is not None else 1 \
            * temporal_super_resolution_steps
        ...
        self.append_blocks = None
        self.use_bias = True
        ch = input_ch = int(channel_mult[0] * self.model_channels)
        self.conv0 = self.relu_layer(in_filters=self.in_channels, out_filters=ch,
                ...
                if ds in attention_resolutions:
                    layers.append(
                        AttentionBlock(
                            ...
                            conf=self.conf,
                            normalization_=normalization2d
                        )
                    )
        ...
        self.out = nn.Sequential(
            normalization2d(ch, conf=self.conf),
            nn.ReLU(),
            zero_module(conv_nd(dims, input_ch, input_ch, 3, padding=1)),
        )

        filters = [input_ch, input_ch, input_ch, input_ch, input_ch, input_ch]
        self.conv_11 = nn.Sequential(
            nn.Conv2d(filters[0] + filters[4] + self.in_channels, filters[5],
        ...
        self.conv_12 = nn.Sequential(
            nn.Conv2d(filters[5], self.out_channels, kernel_size=1, stride=1, padding=0, bias=True))
    ...
    def forward(self, x, timesteps=None, y=None, gt=None, **kwargs):
        """
        ...
        if timesteps[0].item() > self.diffusion_steps_including_temporal_super_resolution_steps:
        ...
        output = self.conv_11(th.cat((h, output_0, x), dim=1))
